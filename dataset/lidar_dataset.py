...
class LiDARDataset(Dataset):
    def __init__(self, config: SHINEConfig, octree: FeatureOctree = None, pose_ref = None) -> None:
    ...
    def process_frame(self, frame_id, cur_pose_ref, incremental_on = False):
        ...
        if cur_pose_ref is not None:
            self.cur_pose = cur_pose_ref
        ...
        frame_origin = self.cur_pose[:3, 3] * self.config.scale  # translation part
        frame_origin_torch = torch.tensor(frame_origin, dtype=self.dtype, device=self.pool_device)

        # transform to reference frame 
        frame_pc = frame_pc.transform(self.cur_pose)
